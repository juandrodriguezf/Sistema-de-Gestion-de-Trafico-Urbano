...
from shared.constants import SENSOR_ESPIRA

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(name)s] %(levelname)s: %(message)s")
logger = logging.getLogger("SensorEspira")


class SensorEspira(Sensor):
    """
    Sensor de espira inductiva — mide el flujo de vehículos (vehículos/minuto).
    Simula el conteo de tráfico realista en una intersección.
    """

    def __init__(self, col: int, row: int, broker_host: str, broker_port: int,
                 frequency: float = 1.0, auth_token: str = "", config: dict = None):
        sensor_id = f"ESP_C{col}K{row}"
        super().__init__(
            sensor_id=sensor_id,
            tipo_sensor=SENSOR_ESPIRA,
            col=col, row=row,
        ...
        # Rangos de datos del config
        ranges = (config or {}).get("data_ranges", {}).get("vehiculos_por_minuto", {})
        self.min_val = ranges.get("min", 0)
        self.max_val = ranges.get("max", 30)
...
if __name__ == "__main__":
    # Uso: python sensor_espira.py <col> <row> [broker_host] [broker_port]
    ...
    token = config.get("auth_tokens", {}).get("sensors_prefix", "") + f"ESP_C{col}K{row}"

    sensor = SensorEspira(col, row, host, port, frequency=freq, auth_token=token, config=config)
    sensor.run()
